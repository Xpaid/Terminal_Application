...
#include <algorithm> // transform function line 1013
...
std::string login()
{
	...
	do
	{
		...
		if (pass.compare("x") || pass.compare("X"))
			return status = "exit";
...
void passing_grade() // #11. Ternary Operator Program (2/3)
{
	while (true)
	{
		...
		std::cout << "                                                                                                                         \n\n";
		grade >= 75 && grade <= 100 ? std::cout << "                                                       " << grade << " is passed\n\n\n"
												<< std::endl
		: grade < 75 ? std::cout << "                                                       " << grade << " is failed\n\n\n"
								 << std::endl
		...
		while (grade_rep != 'y' && grade_rep != 'Y' && grade_rep != 'n' && grade_rep != 'N')
		{
			...
			std::cout << "                                                                                                                     \n\n\n";
			grade >= 77 && grade <= 100 ? std::cout << "                                                       " << grade << " is passed\n\n\n"
													<< std::endl
			: grade > 75 ? std::cout << "                                                       " << grade << " is failed\n\n\n"
									 << std::endl
						 : std::cout << "                                                       " << grade << " is Invalid\n\n\n"
...
int main() // Main function
{
	...
	if (login_res == "granted") // this is abvious
	{
		system("cls");
		// continue & skip the else ifs. go to line 1520
	}

	else if (login_res == "failed") // if not granted is return from the function
	{
		system("cls");
		std::cout << "~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n\n";
		std::cout << "                                                                                                                          \n";
	...
	else if (login_res == "exit") // if exit is return from the function
	{
		system("cls");
		std::cout << "~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n\n";
		std::cout << "                                                                                                                          \n";
	...
	while (true)	 // infinite while loop
	{
		...
		{
		case 'a':
		case 'A':		 // if the user chose 'a' execute this.
			while (true) // makes an infinite loop
			{
				...
				std::cout << "                                                     [x] back                                                             \n\n";
				std::cout << "~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n\n";
				std::cout << "\n\nPlease Select Option: "; // ask
				char operation;							   // declare
				operation = _getch();					   // get the selection store to operation

				if (operation == 'x' || operation == 'X') // if user inputed 'x' break the loop and goto line 1580
				{
					break;
				}
				...
				case '1':
					...
					break;		// after execution of function break, goto line 1579
				case '2':
					calc.minus(); // else minus function line 127
					break;		  // after execution of function break, goto line 1579
				case '3':
					calc.multi(); // else call multi function line 172
					break;		  // after execution of function break, goto line 1579
				case '4':
					calc.div(); // else call the div function line 217
					break;		// after execution of function break, goto line 1579
				} // after this, go to line 1546
			} // while end, goto line 1581
			break; // end of case 'a'. goto line 1523

		case 'b':
		case 'B':
			grocery(); // if the user chose 'b' execute this. goto line 264
			break;	   // end of case 'b'. goto line 1637

		case 'c':
		case 'C':
			average(); // if the user chose 'c' execute this. goto line 419
			break;	   // end of case 'c'. goto line 1637

		case 'd':
		case 'D':
			convert(); // if the user chose 'd' execute this. goto line 486
			break;	   // end of case 'd'. goto line 1637

		case 'e':
		case 'E':
			factorial(); // if the user chose 'e' execute this. goto line 677
			break;		 // end of case 'e'. goto line 1637

		case 'f':
		case 'F':
			sumofNaturalNumbetween(); // if the user chose 'f' execute this. goto line 755
			break;					  // end of case 'f'. goto line 1637

		case 'g':
		case 'G':
			multiplication_table(); // if the user chose 'g' execute this. goto line 845
			break;					// end of case 'g'. goto line 1637

		case 'h':
		case 'H':
			oddeven(); // if the user chose 'h' execute this. goto line 879
			break;	   // end of case 'h'. goto line 1637

		case 'i':
		case 'I':
			passing_grade(); // if the user chose 'i' execute this. goto line 923
			break;			 // end of case 'i'. goto line 1637

		case 'j':
		case 'J':
			palindrome(); // if the user chose 'j' execute this. goto line 974
			break;		  // end of case 'j'. goto line 1637

		case 'k':
		case 'K':
			area(); // if the user chose 'k' execute this. goto line 1050
			break;	// end of case 'k'. goto line 1637

		case 'l':
		case 'L':
			perimeter(); // if the user chose 'l' execute this. goto line 1177
			break;		 // end of case 'L'. goto line 1637

		case 'm':
		case 'M':
			coin_toss(); // if the user chose 'm' execute this. goto line 1346
			break;		 // end of case 'm'. goto line 1637

		case 'n':
		case 'N':
			credits(); // if the user chose 'n' execute this. goto line 1464
			break;	   // end of case 'n'. goto line 1637

		case 'x':
		case 'X':
			break; // exit, this is unnecessary useless code because of line 1539, but just in case lol.. goto line 1637
		} // end of switch case. goto line 1523
	} // end of while loop for 'selection' var
	system("cls"); // this is the exit part of the program
	std::cout << "~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n\n";
	std::cout << "                                                                                                                          \n";
	std::cout << "                                                                                                                          \n"
			  << std::endl;
	...
	std::cout << "                                                                                                                          \n";
	std::cout << "~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n\n";
	system("pause"); // pause for it to be read by the user then press again to exit the program completely
	return 0;
}
